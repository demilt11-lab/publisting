...
const STATUSES: ContactStatus[] = ["not_contacted", "reached_out", "in_talks", "signed", "passed"];
...
export const TeamsPage = ({ onClose, onNavigateToPipeline }: TeamsPageProps) => {
  ...
  const assignmentOverview = useMemo(() => {
    ...
    members.forEach(m => {
      overview[m.user_id] = { not_contacted: 0, reached_out: 0, in_talks: 0, signed: 0, passed: 0 };
    });
    // Add unassigned bucket
    overview["unassigned"] = { not_contacted: 0, reached_out: 0, in_talks: 0, signed: 0, passed: 0 };

    watchlist.forEach(entry => {
      const key = entry.assignedToUserId || "unassigned";
      if (!overview[key]) overview[key] = { not_contacted: 0, reached_out: 0, in_talks: 0, signed: 0, passed: 0 };
        ...
        <div className="rounded-xl border border-border/50 bg-card p-5 space-y-4">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-xs">
              ...
              <tbody>
                {members.map(m => {
                  const counts = assignmentOverview[m.user_id] || { not_contacted: 0, reached_out: 0, in_talks: 0, signed: 0, passed: 0 };
                ...
                {/* Unassigned row */}
                {(() => {
                  const counts = assignmentOverview["unassigned"] || { not_contacted: 0, reached_out: 0, in_talks: 0, signed: 0, passed: 0 };
                  const total = Object.values(counts).reduce((a, b) => a + b, 0);
