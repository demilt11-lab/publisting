...
const STATUSES: ContactStatus[] = ["not_contacted", "watching", "reached_out", "in_talks", "signed", "no_response"];
...
export const TeamsPage = ({ onClose, onNavigateToPipeline }: TeamsPageProps) => {
  ...
  const assignmentOverview = useMemo(() => {
    ...
    members.forEach(m => {
      overview[m.user_id] = { not_contacted: 0, watching: 0, reached_out: 0, in_talks: 0, signed: 0, no_response: 0 };
    });
    // Add unassigned bucket
    overview["unassigned"] = { not_contacted: 0, watching: 0, reached_out: 0, in_talks: 0, signed: 0, no_response: 0 };

    watchlist.forEach(entry => {
      const key = entry.assignedToUserId || "unassigned";
      if (!overview[key]) overview[key] = { not_contacted: 0, watching: 0, reached_out: 0, in_talks: 0, signed: 0, no_response: 0 };
        ...
        <div className="rounded-xl border border-border/50 bg-card p-5 space-y-4">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-xs">
              ...
              <tbody>
                {members.map(m => {
                   const counts = assignmentOverview[m.user_id] || { not_contacted: 0, watching: 0, reached_out: 0, in_talks: 0, signed: 0, no_response: 0 };
                ...
                {/* Unassigned row */}
                {(() => {
                   const counts = assignmentOverview["unassigned"] || { not_contacted: 0, watching: 0, reached_out: 0, in_talks: 0, signed: 0, no_response: 0 };
                  const total = Object.values(counts).reduce((a, b) => a + b, 0);
