...
function Inner(){
  const {data:events=[]}=useQuery({queryKey:["search-telemetry"],queryFn:async()=>{const {data}=await supabase.from("search_events").select("*").order("created_at",{ascending:false}).limit(200);return (data??[]) as SE[];},refetchInterval:30_000});
  ...
  const zMap=new Map<string,number>(); for(const e of events) if(e.zero_result) zMap.set(e.query,(zMap.get(e.query)??0)+1);
