...
const EXPORT_COLUMNS: ExportColumnDef[] = [
  // Order mirrors the on-screen catalog analysis table:
  // Artist → Title → % Controlled → Spotify Streams → YouTube Streams → Release Date
  ...
  { id: "title", label: "Title", group: "core", align: "left", alwaysOn: true, defaultOn: true,
    ...
    csvTotal: () => `""`, pdfTotal: () => "" },
  { id: "splitPercent", label: "% Controlled", shortLabel: "% Controlled", group: "core", align: "right", defaultOn: true,
  ...
  { id: "youtubeViews", label: "YouTube Streams", shortLabel: "YouTube", group: "core", align: "right", defaultOn: true,
...
export default function CatalogAnalysis() {
  ...
  const exportPDF = useCallback(() => {
    ...
    <div class="stats">
      ...
      <div class="stat"><div class="stat-label">Est. Earnings (Gross)</div><div class="stat-value">${formatMoney(analysis.totals.totalIndividualGrossShare)}</div></div>
      <div class="stat"><div class="stat-label">Available to Collect</div><div class="stat-value">${formatMoney(analysis.totals.totalAvailableToCollect)}</div></div>
      <div class="stat"><div class="stat-label">3-Yr Gross Forecast</div><div class="stat-value">${formatMoney(analysis.totals.totalIndividualThreeYearGross)}</div></div>
    ...
    <p style="margin-top:16px;font-size:10px;color:#666">
      ...
      3-Year Forecasts use lifecycle-aware quarterly stream decay (plateau in early stage, ~8% decay mid, ~18% decay late).
    </p>
    <p style="margin-top:8px;font-size:10px;color:#999">Generated by Publisting · ${new Date().toLocaleDateString()}</p>
    </body></html>`);
    w.document.close();
              ...
              <>
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-3">
                  <div className={cardClass + " text-center min-w-0"}>
                    ...
                    <div className="mt-1 text-base md:text-lg font-semibold truncate" title={String(analysis.totals.totalSongsIncluded)}>{formatNumber(analysis.totals.totalSongsIncluded)}</div>
                  </div>
                  <div className={cardClass + " text-center min-w-0"}>
                    <div className={statLabelClass}>Spotify Streams</div>
                    <div className="mt-1 text-sm sm:text-base font-semibold truncate" title={formatNumber(analysis.totals.spotifyStreams)}>{formatNumber(analysis.totals.spotifyStreams)}</div>
                  </div>
                  <div className={cardClass + " text-center min-w-0"}>
                    <div className={statLabelClass}>YouTube Views</div>
                    <div className="mt-1 text-sm sm:text-base font-semibold truncate" title={formatNumber(analysis.totals.youtubeViews)}>{formatNumber(analysis.totals.youtubeViews)}</div>
                  </div>
                  <div className={cardClass + " text-center min-w-0"}>
                    <div className={statLabelClass}>Est. Earnings</div>
                    <div className="mt-1 text-sm sm:text-base font-semibold truncate" title={formatMoney(analysis.totals.totalIndividualGrossShare)}>{formatMoney(analysis.totals.totalIndividualGrossShare)}</div>
                  </div>
                  <div className={cardClass + " text-center min-w-0"}>
                    <div className={statLabelClass}>Available</div>
                    <div className="mt-1 text-sm sm:text-base font-semibold text-primary truncate" title={formatMoney(analysis.totals.totalAvailableToCollect)}>{formatMoney(analysis.totals.totalAvailableToCollect)}</div>
                  </div>
                  <div className={cardClass + " text-center min-w-0"}>
                    <div className={statLabelClass}>3-Year Forecast</div>
                    <div className="mt-1 text-sm sm:text-base font-semibold truncate" title={formatMoney(analysis.totals.totalIndividualThreeYearCollectible)}>{formatMoney(analysis.totals.totalIndividualThreeYearCollectible)}</div>
                  </div>
                </div>
