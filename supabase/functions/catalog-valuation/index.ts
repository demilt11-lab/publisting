...
async function handleBatch(supabase: any) {
  // Get all users with catalog analyses
  ...
  for (const a of analyses) {
    if (!byUser.has(a.user_id)) byUser.set(a.user_id, a);
  }
  ...
  for (const [userId, analysis] of byUser) {
    try {
      ...
      // Get previous valuation to detect growth
  ...
  console.log(`Batch valuation: ${processed} processed, ${alerts.length} alerts`);
...
async function valuateCatalog(supabase: any, userId: string, songs: any[], methodology?: string, assumptions?: any) {
  ...
  const songValuations = songs.map((song: any) => {
    ...
    let value = 0;
    switch (method) {
      case "income_approach": value = calculateDCF(annualRevenue, config); break;
      case "market_multiple": value = annualRevenue * getMedianMultiple(multiples || [], song.genre); break;
      case "monte_carlo": value = runMonteCarlo(annualRevenue, { ...config, simulations: 1000 }).mid; break;
      default: value = annualRevenue * config.multiple;
    }
    return {
  ...
  const { data: valuation } = await supabase.from("catalog_valuations")
    .insert({ user_id: userId, total_value: totalValue, methodology: method, assumptions: config, song_valuations: songValuations, confidence_interval: mcResult })
    .select().single();

  return {
    valuation: valuation || { total_value: totalValue, methodology: method },
    song_valuations: songValuations, confidence_interval: mcResult,
    risk_metrics: { concentration_risk: Math.round(concentrationRisk * 100), genre_diversification: Math.round(Math.min(genres.size / 5, 1) * 100), top_3_percentage: Math.round(concentrationRisk * 100) },
    market_comparables: (multiples || []).slice(0, 5), median_market_multiple: getMedianMultiple(multiples || []),
  };
}

function calculateSongAnnualRevenue(song: any, rates: any[]): number {
  const getRate = (platform: string, country = "US") => {
    const r = rates.find(r => r.platform === platform && r.country_code === country);
...
function getMedianMultiple(multiples: any[], genre?: string): number {
  let f = multiples;
  if (genre) { const gf = multiples.filter(m => m.genre === genre); if (gf.length >= 3) f = gf; }
  if (f.length === 0) return 18;
  const s = f.map(m => m.multiple).sort((a: number, b: number) => a - b);
